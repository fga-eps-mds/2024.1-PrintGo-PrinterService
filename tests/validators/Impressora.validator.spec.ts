import { createImpressoraValidator, updateImpressoraValidator } from '../../src/controllers/validator/Impressora.validator'
...
describe('Impressora Validators', () => {
    ...
    describe('createImpressoraValidator', () => {
        ...
        it('should invalidate enderecoIp if estaNaRede is true but the IP is invalid', () => {
            ...
            const { error } = createImpressoraValidator.validate(invalidData);
            expect(error).toBeDefined();
            expect(error?.details[0].message).toContain("Error code \"custom\" is not defined, your custom type is missing the correct messages definition");
        });
        ...
        it('should invalidate localizacao if it does not have three parts', () => {
            ...
            const { error } = createImpressoraValidator.validate(invalidData);
            expect(error).toBeDefined();
            expect(error?.details[0].message).toContain('localizacao deve conter três partes');
        });
    });
